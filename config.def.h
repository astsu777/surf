...
static char *fulluseragent  = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36"; /* Or override the whole user agent string */
...
#define DLSTATUS { \
        .v = (const char *[]){ "/bin/sh", "-c", \
            "while true; do cat $1/* 2>/dev/null || echo \"There are no downloads\";"\
            "A=; read A; "\
...
static Key keys[] = {
	...
	{ MODKEY,                GDK_KEY_o,      spawn,      SETPROP("_SURF_URI", "_SURF_GO") },
